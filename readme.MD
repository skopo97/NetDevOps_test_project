# Test project for an upcoming bachelor's thesis
...
The project is to set up a virtual GNS3 network bridged to the host client.
...
The automation node will run Ansible, pyATS and a CI runner. The CI runner will receive pipeline jobs when new changes are pushed to the GitHub repository.
...
* GNS3 (latest version)
* Host OS: Linux/Windows (laptop acting as the developer workstation, bridged via a Cloud node)
* Automation control node: Linux VM
* Router and switch images: Cisco IOLv2 (IOS on Unix L2/L3 images)
* Network connectivity between GNS3 and host: Cloud node bridged to the laptop's WAN interface

# Device configurations

The following initial configurations are required on the network devices
...
The GNS3 management network is reachable from the local machine. SSH access has been configured and tested. Initial Ansible connectivity to R1 and S1 was first verified from the developer workstation before migrating the Ansible setup to the dedicated automation control node, which is where it lives going forward.

# Workstation NAT and forwarding configuration
...
Internet access is required for the Automation control node so it can interact with the remote Git repository and retrieve the latest configuration changes, Ansible playbooks, inventor and pyATS scripts.
...
Where in this case `wlp0s20f3` is the internet facing interface on workstation client and `tap0` is the bridged interface to 'GNS3.

The first rule allows the Automation control node to initiate outbound connections to the internet.
The second rule allows return traffic for established connections back to the Automation control node. It does not allow connections to be initiated from the outside network.

The workstation itself does not run Ansible and does not need SSH access to R1 or S1. It's role is limited to NAT, forwarding, and acting as the default gateway for the automation control node.
...
pyATS will be used as part of the validation stage to automate network tests. If the tests pass, the pipeline can proceed with pushing the configuration changes to the managed devices. pyATS will be installed in a Python virtual environment on the control node (see pyATS configuration section below).
...
The first playbook verifies that Ansible can connect to the network devices and can run commands. The playbook is located at `ansible/playbooks/show_version.yml`
...
Create a Python3 virtual environment on the control node, activate it, and install pyats[full] via pip.
...
This requires creating a .env file in the directory with two variables
`ssh_user=x` and `ssh_pass=x`

### Testing pyATS connectivity

pyATS connectivity is first tested with a Python script. The script connects into R1 and runs the `show version` command on the device and outputs it to the terminal.
...
The script successfully connects to device R1 and runs the 'show version' command. This confirms that connectivity is working.

# Manual pipeline Testing
#
 Before moving to the next logical step in the project. The idea is to first test how the pipeline should work when automated. 

 The first step is to crate a Ansible playbook for configuration changes and to create a pyATS script that validates connectivity.
